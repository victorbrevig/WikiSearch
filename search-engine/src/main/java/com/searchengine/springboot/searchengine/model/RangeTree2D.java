...
public class RangeTree2D {
    ...
    RangeTree2D(Integer[] X, Integer[] Y, HashTable<Integer, String> idToString, HashTable<Integer, String> idToRevString) {
    	this.X = X;
    	this.Y = Y;
    ...
    public Pair<ArrayList<Integer>,ArrayList<YarrayResult>> rangeSearch2D(int x1ID, int x2ID, int y1ID, int y2ID) {
    	Pair<ArrayList<Integer>,ArrayList<YarrayResult>> res = new Pair(new ArrayList<Integer>(),new ArrayList<YarrayResult>());
    	
    	String x1 = idToString.search(x1ID);
    	String x2 = idToString.search(x2ID);
    	String y1Rev = idToRevString.search(y1ID);
    	String y2Rev = idToRevString.search(y2ID);
    	
    	System.out.println("RANGE SEARCH");
    	System.out.println("x1: " + x1);
    	System.out.println("x2: " + x2);
    	System.out.println("y1: " + idToString.search(y1ID));
    	System.out.println("y2: " + idToString.search(y2ID));
    	
    	// check if 1D problem
    	if(x1ID == x2ID) {
    		if(isInYrange(x1ID, y1Rev, y2Rev)) {
    			res.first.add(x1ID);
    		}
    		return res;
    	}
    	else if(y1ID == y2ID) {
    		if(isInXrange(y1ID, x1, x2)) {
    			res.first.add(y1ID);
    		}
    		return res;
    	}
	...
	private Pair<ArrayList<Integer>,ArrayList<YarrayResult>> collectNodesLeft(BSTnode node, Pair<ArrayList<Integer>,ArrayList<YarrayResult>> acc, String x1, int y1Pos, int y2Pos, String y1Rev, String y2Rev) {
    	...
    	if(nodeStr.equals(x1)) {
    		...
    		if(node.right != null) {
    			...
    			if(isInYrange(node.right.data, y1Rev, y2Rev)) {
	...
	private Pair<ArrayList<Integer>,ArrayList<YarrayResult>> collectNodesRight(BSTnode node, Pair<ArrayList<Integer>,ArrayList<YarrayResult>> acc, String x2, int y1Pos, int y2Pos, String y1Rev, String y2Rev) {
    	...
    	if(nodeStr.equals(x2)) {
    		...
    		if(node.left != null) {
    			...
    			if(isInYrange(node.left.data, y1Rev, y2Rev)) {
	...
	private boolean isInXrange(int strId, String x1, String x2) {
		String str = idToString.search(strId);
		
		return str.compareTo(x1) >= 0 && str.compareTo(x2) <= 0;
	}
    ...
    public static void main(String[] args) {
    	...
    	/*
    	idToWord.insert(9,"an-");
    	idToWord.insert(10,"an");
    	idToWord.insert(11,"Stoicism");
    	idToWord.insert(12,"marxism");
    	*/
    	...
    	/*
    	idToRevWord.insert(11,IndexTools.revString("Stoicism"));
    	idToRevWord.insert(9,IndexTools.revString("an-"));
    	idToRevWord.insert(12,IndexTools.revString("marxism"));
    	idToRevWord.insert(10,IndexTools.revString("an"));
        ...
        */
    	...
    	RangeTree2D tree = new RangeTree2D(X, Y, idToWord, idToRevWord);
        /*
    	...
    	*/
    	...
    	Pair<ArrayList<Integer>, ArrayList<YarrayResult>> res = tree.rangeSearch2D(8, 5, 7, 6);
